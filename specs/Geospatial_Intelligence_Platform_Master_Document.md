...
> **As built (Phase 4).** Implemented with a **single LLM call made directly** (no AI Gateway — avoids extra moving parts/cost). The layer is provider-swappable via one `AI_PROVIDER` flag in `src/config.ts`: **Groq `gpt-oss-120b`** is active (validated 2026-07-13 as ~5× faster than OpenAI `gpt-5-mini` — median ~1.5 s vs ~7 s — at equal/better structured quality); OpenAI `gpt-5-mini` is the fallback. Structured Outputs (strict json_schema) on both; briefing text is Spanish. Reasoning is kept low/minimal because the deterministic engine already did the analysis — an explicit priority rubric + per-field guidance in the prompt carries the load. Called once per event (only when it has no briefing yet), best-effort and self-healing (a failed call is retried on the next engine pass), capped per run. Output schema adds a `briefing_text` field so one call yields both structured JSON and prose. See [`docs/ai-briefing.md`](../docs/ai-briefing.md).
